...
test.describe("PROH — גבולות המודל", () => {
  ...
  test("PROH-20/BR-39 — נמען ללא טלפון וללא מייל נחסם", async ({ page }) => {
    ...
    // הנוסח עודכן ב-1.9.2026 לזה שבאפיון (GAP-A5); הטענה כאן היא על
    // **החסימה**, ולכן היא נשענת על הפיקסצ׳ר ולא על ניסוח משלה.
    await expect(
      page.getByRole("alert").filter({ hasText: RECIPIENTS_SCREEN.missingContact }),
    ).toBeVisible();
  ...
  test("S3-08 — נוסח ההודעה על נמען בלי פרטי קשר, כלשונו באפיון", async ({ page }) => {
    /**
     * **היה `test.fail()` עד 1.9.2026 (GAP-A5).**
     *
     * ‏§4 מסך 3 מחייב את המשפט כלשונו. הקוד החזיק במקומו נוסח משלו
     * ("לא ניתן לשגר: לנמען אין טלפון ואין מייל") — התנהגות **נכונה**
     * בניסוח **אחר**, שהוא בדיוק סוג הפער שחבילת ההתאמה קיימת כדי לתפוס:
     * הוא אינו מפיל שום בדיקה פונקציונלית, ואיש אינו מדווח עליו.
     *
     * ‏`contactRequiredHint` נשאר בנוסח שונה במכוון — הוא רמז **מונע**
     * שנתלה בשדה מראש, לא הודעת הכשל. שני משפטים זהים על אותו מסך היו
     * גם רעש וגם בדיקה שעוברת בלי שהחסימה עובדת.
     */
    await loginAs(page, "managerA");
    await page.goto("/tickets/new");
    await page.getByRole("button", { name: "+ איש מקצוע חדש" }).click();
