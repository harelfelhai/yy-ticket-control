 ...
 */
const NAV_LINK =
  "flex min-h-8 shrink-0 items-center whitespace-nowrap rounded-sm px-2 font-medium text-fg";
      ...
      >
        ...
        */}
        <nav className="flex min-w-0 flex-1 items-center justify-end gap-3 overflow-x-auto text-sm">
          ...
          {/*
           * **החשבון האישי (1.1) — שם המשתמש הפך לקישור, ולא נוסף פריט לסרגל.**
           *
           * שתי הכרעות מתועדות נפגשות כאן, ושתיהן נשמרות. "הסרגל מחזיק שני
           * קישורים ולא חמישה" (§ סרגל ניווט) הוריד משם את "תגיות" בנימוק
           * "נכנסים אליה לעיתים רחוקות; קישור קבוע היה מחיר על כל מסך" — נימוק
           * שחל מילה במילה על מסך שמחליפים בו סיסמה פעם בשנה. לכן היעד החדש
           * **רוכב על אלמנט שכבר היה כאן** במקום להיווסף לצדו.
           *
           * ‏`text-fg` ולא `text-muted`: זהו קישור ניווט, ו-`text-muted` הוא
           * הסימן שמייחד את היציאה כפעולה שאינה ניווט (§ סרגל ניווט). כשזה
           * היה `<span>` של טקסט מת האפור היה נכון; משהפך ליעד — אינו.
           *
           * **ההסתרה במסך צר ירדה, וזו הפיכה של ההחלטה הקודמת.** הנימוק שלה
           * היה "הוא מידע ולא פעולה", והוא חדל להתקיים ברגע שהוא הפך לפעולה.
           * פעולה שקיימת רק בדסקטופ אינה קיימת — ומנהל עבודה, שאינו רואה את
           * הקישור ל-`/admin` כלל, היה נשאר בלי שום דרך להחליף סיסמה.
           */}
