...
interface FilterBarProps {
   ...
   * אילו פרמטרים הם מסננים ואילו הם מצב תצוגה (`tour`, `focus`).
   */
  activeCount: number;
  /**
   * פקדים שנשארים גלויים גם כשהרצועה מקופלת. שם מקומם של דברים שאינם
   * מסננים — מתג "מצב סיור" הוא מצב תצוגה ולא סינון, והוא הפקד השכיח
   * ביותר בשטח.
   */
  trailing?: ReactNode;
  children: ReactNode;
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      {/*
        ...
        (`w-full`), ובלעדיו היא הייתה דוחפת את "מצב סיור" אל מתחתיה. במסך
        רחב הפאנל חוזר לסדר ה-DOM ומשתלב באותה שורה עם שאר הפקדים.
      */}
      <div
        id={panelId}
        className={twMerge(
