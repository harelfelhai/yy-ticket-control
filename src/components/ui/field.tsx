...
/**
 * **הגובה אינו תלוי במכשיר, ה-radius אינו תלוי בכלום.**
 *
 * ‏32px ו-28px, בטלפון ובעכבר כאחד — כמו בכפתור, ומאותה הכרעה: רצפת המגע
 * של 44px בוטלה ב-0.7 (`docs/DESIGN.md` § אזורי מגע). ה-radius ירד ל-4px
 * בכל הגדלים — שדה ופקד מסנן שיושבים זה לצד זה נבדלו קודם גם בעיגול, וזו
 * הבחנה שאיש לא התכוון אליה.
 */
const CONTROL_SIZES: Record<ControlSize, string> = {
  default: "min-h-8 px-2",
 ...
 *
 * **‏`py-0` ב-`compact`, וזו מדידה ולא טעם.** שורת `text-base` היא 25.6px
 * (גובה שורה 1.6, ‏`globals.css`), ועם `p-2` קבוע התיבה יוצאת ‏≈43.6px —
 * כלומר **‏11px גבוהה מהכפתור הקומפקטי שלצדה בקומפוזר**, וזה בדיוק הפער
 * שדווח. ‏`py-0` מחזיר אותה ל-28px, גובה הכפתור שלצדה.
 *
 * ‏`touch:py-2` ירד יחד עם רצפת המגע (0.7): הוא היה קיים רק כדי שטקסט לא
 * יידבק לראש קופסה של 44px — ‏`<textarea>` מיישר לראש ולא למרכז — ואין
 * יותר קופסה כזו.
 */
const TEXTAREA_SIZES: Record<ControlSize, string> = {
  default: "p-2",
  compact: "min-h-7 px-2 py-0",
};
