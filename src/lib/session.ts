 ...
 * הרשמה עצמית וללא איפוס סיסמה בדוא״ל. ספרייה גדולה הייתה מוסיפה טבלאות,
 * מסכים ותלות שאיש לא ישתמש בהם.
 *
 * **‏1.2 הוסיף ספק זהות חיצוני אחד — Google — והשורה תוקנה ולא נמחקה.**
 * קודם נכתב כאן "ללא ספקי זהות חיצוניים", וזה היה חלק מהנימוק. הנימוק
 * **עמד במבחן**: מסלול ה-OIDC כולו הוא שלושה מודולים (`google-oauth.ts`,
 * `oauth-state.ts`, `services/google-login.ts`), והוא נגמר בקריאה ל-
 * ‏`createSession()` עם אותו `SessionUser` בדיוק — בלי טבלה, בלי מסך, ובלי
 * צורת סשן שנייה. מה שספריית auth מלאה הייתה מוסיפה כאן הוא
 * ‏Account/Session/VerificationToken וכל מה שנגזר מהן.
 *
