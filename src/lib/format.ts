/**
 * פורמט לתצוגה — זמן, ובסופו גם טקסט.
 *
...
/**
 * השורה הראשונה מתיאור הפנייה, לתצוגה ברשימה.
 *
 * **רשימה אינה מקום לקרוא בו תיאור מלא**, והתיאור הוא טקסט חופשי שנכתב
 * בשטח — לעיתים כמה פסקאות. השורה הראשונה היא כמעט תמיד מה שהתקלה, ושאר
 * הטקסט הוא פירוט.
 *
 * החיתוך ב-120 תווים נוסף מעל השורה הראשונה: מי שכותב הכול בשורה אחת אינו
 * מקבל טיפול שונה. הוא נעשה **בשרת** ולא ב-`truncate` של CSS בלבד, כדי
 * שהרשת לא תישא פסקה שלמה לכל שורה ברשימה של עשרים.
 *
 * ‏**מקור אמת אחד** (0.5): הפונקציה ישבה בשני עותקים זהים תו-בתו,
 * ב-`services/board.ts` וב-`services/search.ts`, והצרכן השלישי (מסך התגית)
 * הוא שהפך את הכפילות לעלות אמיתית.
 */
