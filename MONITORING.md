...
**האירועים האלה אינם ב-`railway logs`.** `logInfo`/`logWarn` כותבים ל-Sentry
Logs בלבד, וה-stdout של הקונטיינר מציג רק את העלייה ואת Prisma. מי שבודק את
הקליטה מתוך המסוף — למשל אחרי הדלקה או פריסה — שואל את בסיס הנתונים ישירות
(`DATABASE_PUBLIC_URL`, בטרנזאקציה `READ ONLY`):

| שאלה | מקור |
|---|---|
| הטיימר חי, והתיבה והרשאה תקינות? | `MailChannelState`: `lastPollOkAt` בן פחות מדקתיים, `lastPollError` ריק |
| מתי הופעלה הקליטה (הגבול של EM-22)? | `MailChannelState.activatedAt` |
| כל מייל שנקלט הוכרע? | `MailboxMessage` נכנסות: `state = DONE` ו-`outcome`; PENDING ותיק הוא תקיעה |
| התשובה עמדה בחמש הדקות? | שורה יוצאת `sentAt` פחות `receivedAt` של השורה שב-`repliesToId` |

> עמודות הזמן הן `timestamp` **בלי** אזור זמן, ב-UTC. דרייבר שמפרש אותן כשעון
> מקומי (למשל `pg` ב-Node על מחשב בישראל) מציג אותן מוזזות בשעתיים-שלוש.
> `to_char(...)` בתוך השאילתה מחזיר את הערך כפי שנשמר.
