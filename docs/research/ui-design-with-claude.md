...
**ממצא בפרויקט — ותיקון:** סריקה ראשונה דיווחה על 29 מופעים ב-13 קבצים. **המספר היה שגוי:**
הדפוס כלל `rounded-[tb]?[lr]`, שתופס את ה-`l` שבתוך `rounded-lg` — כלומר רוב המופעים היו
מחלקות עיגול תקינות. הספירה המדויקת היא **3**: `left-4` על ה-FAB (חריג לגיטימי ומתועד),
ושני `mr-2` שמתרנדרים נכון ב-RTL כי `margin-right` שם שקול ל-`margin-inline-start`.

זהו לקח מתודולוגי ולא רק תיקון מספר: **grep על מחלקות Tailwind דורש עוגני גבול.** מחלקה
היא מחרוזת בתוך מחרוזת ארוכה יותר, ודפוס בלי `\b` או בלי דרישת תו מפריד מייצר ממצאי-שווא
בסדר גודל. בפרויקט הזה, שבנוי מלכתחילה RTL-first עם `he-IL` ו-`Intl`, ה-RTL אינו הפער
שחשבתי שהוא — והבדיקה האוטומטית שכן שווה להריץ היא `mobile-qa.spec.ts` הקיימת, שבודקת
גלישה אופקית תחת WebKit.
...
**שלב 2 — RTL: בוטל אחרי בדיקה (~10 דקות)**

הפער הזה תוכנן כחצי יום עבודה על סמך ספירה שגויה. בפועל נותרו שני `mr-2` שמתרנדרים נכון,
והמלצת ה-`rtlify-ai` אינה מוצדקת כאן — התקנת כלי וחוקים לתיקון שני מופעים היא עלות בלי
תמורה. מה שכן נשאר:

5. להחליף את שני ה-`mr-2` ב-`ms-2` — עקביות, אפס שינוי ויזואלי.
6. טבלת המיפוי הפיזי→לוגי כבר נוספה ל-`AGENTS.md` ול-`DESIGN.md` § Layout. **זה החלק
   שבאמת חשוב**: הוא מונע את הפער בקוד חדש, שם סוכן שמאומן על LTR באמת נוטה לשגות.

הלקח הכללי: **לוודא ממצא לפני שמתכננים לפיו.** הפער הזה נכנס לתוכנית עם דירוג "גבוהה"
בלי שנפתח ולו קובץ אחד לבדוק אם הוא באג.
...
- ❌ **לא** להחליף את גופן המערכת בגופן "מעניין". בעברית מרחב הגופנים הקריאים צר בהרבה
  מאנגלית, והבעיה בטיפוגרפיה כאן היא **היעדר סקאלה**, לא הגופן.
  > **עודכן 18.8.2026:** בזמן כתיבת המסמך הגופן היה Heebo, והשורה נקבה בשמו. הוא הוחלף
  > מאז ב-**Assistant** (`docs/DESIGN.md` § Typography). ההסתייגות כאן תקפה כלשונה —
  > היא מכוונת נגד גופן דקורטיבי, ו-Assistant הוא גופן ממשק. אין לקרוא את השורה הזו
  > כאיסור על ההחלפה שכבר בוצעה.
