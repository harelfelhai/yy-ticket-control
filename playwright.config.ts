 ...
 *
 * שתי הכרעות:
 * - פורט 3101 ובסיס נתונים ייעודי ל-E2E (`yy_e2e`), ולא 3100 ובסיס הפיתוח — כדי
 *   לא לזהם נתוני פיתוח, וכדי לא להתנגש בבדיקות האינטגרציה — שמרוקנות
 *   את הבסיס שלהן בין בדיקות ולכן היו משאירות כאן ישויות זרות.
 *
 *   **הפורט הנפרד כבר אינו מאפשר להריץ במקביל לשרת הפיתוח.** עד 0.8
 *   נכתב כאן שזו המטרה, וזה הפסיק להיות נכון: ‏Next 16 חוסם `next dev` שני
 *   **באותה תיקייה** ולא באותו פורט — `Another next dev server is already running` —
 *   ולכן הרצת E2E דורשת לעצור את שרת הפיתוח תחילה. מה שנשאר מהנימוק
 *   המקורי הוא הבידוד: בסיס נתונים נפרד, עוגיות נפרדות, ותצורה שאינה
 *   נוגעת בסביבת העבודה. מי שרוצה באמת להריץ במקביל צריך `E2E_PRODUCTION=1`,
 *   שמריץ `next build && next start` — ועליו החסימה אינה חלה.
...
export default defineConfig({
  ...
  webServer: {
    ...
    env: {
      ...
      GEMINI_API_KEY: "",
      /**
       * ערכי דמה, ומפורשים מאותה סיבה של `GEMINI_API_KEY` — אלא שכאן
       * הכיוון הפוך: החבילה בודקת את המסלול **המוגדר**, ולכן היא חייבת
       * לשלוט בשאלה האם הוא מוגדר. בלי זה הבדיקות היו נשענות על מה שיש
       * ב-`.env` של המכונה, כלומר עוברות אצל מי שהגדיר גוגל ונופלות אצל
       * מי שלא.
       *
       * ‏**שום בדיקה אינה פונה לגוגל בפועל.** מה שנבדק הוא לאן *אנחנו*
       * שלחנו את הדפדפן ומה שמנו בעוגייה — הטענות הן על ה-`Location`
       * ועל ה-`Set-Cookie`, ולא על מה שגוגל עשתה. ערך אמיתי בסביבה גובר,
       * כדי שסבב ידני מול גוגל יהיה אפשרי בלי לגעת בקובץ.
       */
      GOOGLE_CLIENT_ID:
