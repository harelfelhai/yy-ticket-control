 ...
 *
 * **הבעיה שזה פותר.** הפריסה אוטומטית מ-push ל-`main` (מ-15.8.2026), אבל
 * "נפרס" אינו "עלה": בנייה שנכשלת, healthcheck שאינו עובר או פריסה שנתקעת
 * משאירים את האתר על הגרסה הקודמת — ואז `main` ירוק, נקי ודחוף חי לצד אתר
 * שמריץ קוד ישן. אף בדיקה בפרויקט לא הייתה נכשלת: כולן רצות מול בנייה
 * מקומית. מה שנחסר הוא בדיקה שמשווה את **מה שמוגש** למה שכתוב בריפו.
 *
 * > **תיקון נוסח.** כאן נכתב עד 26.8.2026 ש"הפריסה כאן ידנית ואינה מופעלת
 * > מ-push". זה היה נכון כשהסקריפט נכתב, וחדל להיות נכון ב-15.8.2026 כשחובר
 * > הריפו. אומת מול `railway deployment list`. **הסקריפט עצמו לא איבד את
 * > תפקידו** — רק הנימוק התחלף מ"אולי לא פרסת" ל"אולי הפריסה נכשלה".
 *
