...
describe("§6 — מה שנדחה במפורש אינו קיים במערכת", () => {
   ...
   */
  /**
   * **הורחב ב-1.2, וזו הקשחה ולא ריכוך.**
   *
   * ‏1.2 הוסיף כניסה עם חשבון Google — כלומר מ-`accounts.google.com` נעשה
   * מארח לגיטימי בקוד, והגבול חדל להיות "אין גוגל" ונעשה **"גוגל לזהות
   * בלבד"**. איסור שאינו מבחין ביניהם היה מתנגש בדרישה קיימת, וזה בדיוק מה
   * שקרה ב-1.9.2026 כשהניסוח תפס את נקודת הקצה של Gemini.
   *
   * הגבול החדש נאכף בשני קצותיו:
   * - **שלילית ורחבה מקודמתה:** אף `scope` של גוגל אינו מותר. הנתיב
   *   `googleapis.com/auth/` אסור כולו, ולא רק `auth/calendar` — כך שגם
   *   `auth/drive` או `auth/contacts` ייתפסו, ולא רק היומן שנדחה במפורש.
   *   ‏Gemini אינו מתנגש: כתובתו היא `.../v1beta/...`, בלי `auth/`.
   * - **חיובית:** ההיקף שהמערכת כן מבקשת מוצהר בקוד. בלעדיה, החלפת
   *   `GOOGLE_SCOPES` למחרוזת אחרת הייתה משאירה את האיסור נבדק על ריק.
   */
  it("SC-OUT-02 — גוגל לזהות בלבד: אין יומן ואין שום scope אחר", () => {
    expect(source).not.toMatch(
      /calendar\.google|google-auth-library|googleapis\.com\/calendar|auth\/calendar|calendar\.events|google\.calendar/i,
    );
    // אף scope של גוגל, ולא רק זה של היומן.
    expect(source).not.toMatch(/googleapis\.com\/auth\//i);

    // הכיוון החיובי: אם Gemini ייעלם, האיסור לא ייבדק על ריק.
    expect(source).toMatch(/generativelanguage\.googleapis\.com/);
    // וההיקף של ההתחברות (1.2) מוצהר, ולכן ניתן לביקורת.
    expect(source).toMatch(/openid email profile/);
  });
...
describe("§3 — הסכימה תואמת את מודל המידע", () => {
  /**
   * **ארבעה ערכים מ-1.3** (§3.2 שדה 13): `ממייל` נוסף כשפתיחת פנייה במייל
   * נכנסה לתחולה. הרשימה נעולה מאותה סיבה כמו קודם — ערך שנוסף בלי החלטה
   * באפיון הוא ערוץ שאיש לא הגדיר את כלליו.
   */
  it("DM-F13 — ערוץ המקור הוא ארבעת הערכים של האפיון", () => {
    const values = (SCHEMA.match(/enum Channel \{([^}]+)\}/)?.[1] ?? "")
      .split(/\s+/)
      .filter(Boolean);
    expect(values).toEqual(["SELF", "MANAGEMENT", "WHATSAPP", "EMAIL"]);
  });
