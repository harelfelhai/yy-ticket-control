 ...
 * ---
 *
 * **גבול שנקבע ב-1.0: `src/lib/use-has-keyboard.ts` שואל `any-pointer: fine`,
 * וזו אינה עקיפה של האיסור כאן.**
 *
 * מה שנאסר למעלה הוא `pointer-coarse:` — כלומר השאלה **השבורה**, זו
 * ששואלת על המצביע הראשי ולכן עונה "גס" על לפטופ עם מסך מגע. `RETIRED_QUERY`
 * למטה מעיד על כך בעצמו: גם הווריאנט שבוטל נאלץ לצרף
 * `and (not (any-pointer: fine))` בדיוק כדי לתקן את אותו פגם. ההוק החדש
 * שואל את החצי הזה **לבדו**, שהוא הצורה הנכונה.
 *
 * ושתי הבחנות מהותיות נוספות:
 *
 * - **מה שמותנה אינו גיאומטריה אלא התנהגות.** האיסור כאן שומר על
 *   "גובה הפקד זהה בכל מכשיר" (הכרעת בעל המוצר, 0.7), ואף פיקסל אינו זז
 *   בגלל ההוק. מה שמשתנה הוא מה עושה מקש Enter — הכרעה נפרדת של אותו
 *   בעל מוצר (7.9.2026), שנוסחה במפורש כ"במחשב שולח, בטלפון יורד שורה".
 * - **וזה כן נבדק בדפדפן**, בניגוד לפסקה שמעליה. הטענה שם נכונה למקרה
 *   ההיברידי בלבד; שני המקרים הטהורים — Pixel 5 מול Desktop Chrome —
 *   מדומים נכון, ושניהם נאכפים ב-`e2e/ticket-actions.spec.ts`.
 */
