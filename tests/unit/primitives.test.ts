...
const EXEMPT: Record<string, string> = {
  ...
  // ‏`success` אינו וריאנט של `Button`, ובכוונה: אתר קריאה אחד בלבד.
  // **תיל מתיחה:** אתר שני הוא ההצדקה להוסיף אותו, בדיוק כפי ש-`quiet`
  // נוסף בדיעבד.
  //
  // עד 0.4 ישבו כאן שניים — `warning` היה של "יש לי שאלה", שהוסר יחד עם
  // סטטוס QUESTION. הרשומה צומצמה במקום להישאר כפי שהייתה: חריגה שמתארת
  // קוד שאינו קיים היא בדיוק מה שמרשה לחריגה הבאה להיכנס בלי דיון.
  "app/p/[token]/[ticketId]/portal-actions.tsx":
    "כפתור אחד: 'סיימתי — טופל' ב-success ובגובה 56px",
